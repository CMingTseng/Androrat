			...
			switch (command) 
			{
				case Protocol.CONNECT:
					...
					if(!c.getClientMap().containsKey(new_imei))
					{
						...
						//on efface ses donnes tomporaires attribues  la connexion
					...
					//si le client s'est reconnect (imei dja inscrit)
					else
					{
						//on rcupre son gestionnaire
						...
						c.getClientMap().remove(temp_imei);
						c.getChannelHandlerMap().remove(temp_imei);
						//et lors de la connexion prcdente!
